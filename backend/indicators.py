...
class Vue(str, Enum):
    PROFIL = "profil"   # mesurable seulement de cote
    FACE = "face"       # mesurable seulement de face ou trois-quarts
...
C01 = Indicateur(
    id="C01", nom="variante", phase=Phase.CONTEXTE, source=Source.POSE, portee=Portee.SET,
...
C02 = Indicateur(
    id="C02", nom="vue_camera", phase=Phase.CONTEXTE, source=Source.POSE, portee=Portee.SET,
    question="Where is the camera relative to the lifter?",
    etats=(Etat("profil", "Filmed from the side: bar path and joint angles are readable."),
           Etat("trois_quarts", "Three-quarter view: angles are projected and only "
                                "large changes are meaningful."),
           Etat("face", "Filmed from the front: knee tracking is readable, bar path is not.")),
    mesure="vue", seuils=((0.30, "profil"), (0.60, "trois_quarts"), (INF, "face")),
...
C03 = Indicateur(
    id="C03", nom="qualite_pose", phase=Phase.CONTEXTE, source=Source.POSE, portee=Portee.SET,
    question="How reliable are the body landmarks over the clip?",
    etats=(Etat("bonne", "Landmarks are stable and visible throughout."),
           Etat("moyenne", "Landmarks are usable but drop out on part of the clip."),
           Etat("faible", "Landmarks are unreliable: measurements are suspended.")),
    mesure="visibilite", seuils=((0.40, "faible"), (0.70, "moyenne"), (INF, "bonne")),
...
C04 = Indicateur(
    id="C04", nom="materiel", phase=Phase.CONTEXTE, source=Source.LLM, portee=Portee.SET,
    question="What is being lifted, and with what equipment?",
    etats=(Etat("barre_libre", "A standard barbell with plates on the floor."),
           Etat("trap_bar", "A trap bar / hex bar: the lifter stands inside the frame."),
           Etat("autre", "Something else: dumbbells, Smith machine, blocks, deficit, "
...
C05 = Indicateur(
    id="C05", nom="prise", phase=Phase.CONTEXTE, source=Source.LLM, portee=Portee.SET,
    question="How does the lifter grip the bar?",
    etats=(Etat("double_pronation", "Both palms facing the lifter (double overhand)."),
           Etat("mixte", "One palm forward, one back (mixed grip)."),
           Etat("crochet", "Hook grip: the thumb is trapped under the fingers."),
           Etat("sangles", "Lifting straps are used.")),
...
S01 = Indicateur(
    id="S01", nom="hauteur_hanches", phase=Phase.SETUP, source=Source.POSE, portee=Portee.REP,
    critere="setup", vue=Vue.PROFIL,
    question="How high are the hips at the start, between the knees and the shoulders?",
    etats=(Etat("trop_hautes", "The hips start very high: the pull begins as a stiff-legged "
                               "lift with the shoulders far in front.", 2),
           Etat("optimale", "The hips sit between the knees and the shoulders.", 3),
           Etat("trop_basses", "The lifter squats the setup: pelvis very low, shins pushed "
                               "forward, knees over the bar.", 2, persona="The Squatter")),
    mesure="hanches_ratio", seuils=((0.40, "trop_hautes"), (0.80, "optimale"), (INF, "trop_basses")),
...
S02 = Indicateur(
    id="S02", nom="epaules_sur_barre", phase=Phase.SETUP, source=Source.POSE, portee=Portee.REP,
    critere="setup", vue=Vue.PROFIL,
    question="Where are the shoulders relative to the bar at the start?",
    etats=(Etat("derriere", "The shoulders start behind the bar, which sends the bar forward "
                            "as soon as it leaves the floor.", 2),
           Etat("au_dessus", "The shoulders are stacked over or just ahead of the bar.", 3),
           Etat("tres_avant", "The shoulders are far ahead of the bar, lengthening the "
                              "lever on the lower back.", 2)),
    mesure="epaules_barre", seuils=((-0.08, "derriere"), (0.30, "au_dessus"), (INF, "tres_avant")),
...
S03 = Indicateur(
    id="S03", nom="angle_tibia", phase=Phase.SETUP, source=Source.POSE, portee=Portee.REP,
    critere="setup", vue=Vue.PROFIL, variantes=("sumo",),
    question="How vertical are the shins at the start? (sumo)",
    etats=(Etat("verticaux", "The shins are vertical or nearly so: the wedge is in place.", 3),
           Etat("inclines", "The shins lean forward, pushing the knees over the bar.", 2)),
    mesure="tibia_deg", seuils=((20.0, "verticaux"), (INF, "inclines")),
...
S04 = Indicateur(
    id="S04", nom="barre_milieu_pied", phase=Phase.SETUP, source=Source.A_TESTER, portee=Portee.REP,
    critere="setup", vue=Vue.PROFIL,
    question="Where is the bar over the foot at the start?",
    etats=(Etat("milieu_pied", "The bar sits over the middle of the foot, close to the shins.", 3),
           Etat("trop_avant", "The bar starts away from the shins, ahead of the mid-foot.", 2),
           Etat("sur_les_tibias", "The bar starts jammed against the shins, behind the mid-foot.", 3)),
...
S05 = Indicateur(
    id="S05", nom="dos_au_setup", phase=Phase.SETUP, source=Source.LLM, portee=Portee.REP,
    critere="spine",
    question="What shape is the back in before the bar moves?",
    etats=(Etat("neutre", "The back is flat and set before the bar moves.", 3),
           Etat("haut_arrondi", "The upper back is rounded but the position looks deliberate "
                                "and set.", 3),
           Etat("lombaire_arrondi", "The lower back is already rounded at the start.", 2,
...
S06 = Indicateur(
    id="S06", nom="bras_tendus", phase=Phase.SETUP, source=Source.A_TESTER, portee=Portee.REP,
    critere="setup",
    question="Are the arms straight at the start, or are the elbows actively bent?",
    etats=(Etat("tendus", "The arms hang straight, elbows locked out.", 3),
           Etat("flechis", "The elbows are visibly flexed, loading the biceps tendon.", 1,
                persona="The T-Rex")),
...
S07 = Indicateur(
    id="S07", nom="mise_en_tension", phase=Phase.SETUP, source=Source.LLM, portee=Portee.REP,
    critere="setup",
    ...
    etats=(Etat("progressive", "The arms pull taut and the bar or plates visibly load "
           ...
           Etat("partielle", "Some tension is taken but it is lost as the bar breaks the floor.", 2),
           Etat("a_coup", "No pre-tension at all: the lifter yanks the bar off the floor "
...
S08 = Indicateur(
    id="S08", nom="orientation_pieds", phase=Phase.SETUP, source=Source.LLM, portee=Portee.SET,
    question="How are the feet oriented?",
    etats=(Etat("ouverts", "The toes are flared outwards."),
           Etat("droits", "The toes point roughly forward.")),
...
L01 = Indicateur(
    id="L01", nom="montee_hanches_epaules", phase=Phase.DECOLLAGE, source=Source.POSE,
    ...
    etats=(Etat("ensemble", "Hips and shoulders rise together: the legs are driving the "
                            "floor away and the torso angle holds.", 3),
           Etat("hanches_devant", "The hips rise somewhat ahead of the shoulders, but the "
                                  "torso does not pitch forward.", 3),
           Etat("hanches_fusent", "The hips shoot up while the shoulders barely move: the "
                                  "lift turns into a stiff-legged pull finished by the back.",
                1, persona="The Crane")),
    mesure="ratio_montee", seuils=((1.60, "ensemble"), (2.60, "hanches_devant"), (INF, "hanches_fusent")),
...
L02 = Indicateur(
    id="L02", nom="bascule_buste", phase=Phase.DECOLLAGE, source=Source.POSE,
    portee=Portee.REP, critere="leg_drive", vue=Vue.PROFIL,
    question="Does the torso pitch further forward as the bar breaks the floor?",
    etats=(Etat("stable", "The torso angle holds as the bar leaves the floor.", 3),
           Etat("bascule", "The torso pitches further forward at liftoff: the hips win "
                           "the race and the back takes the load.", 2)),
    mesure="bascule_deg", seuils=((8.0, "stable"), (INF, "bascule")),
...
L03 = Indicateur(
    id="L03", nom="a_coup_depart", phase=Phase.DECOLLAGE, source=Source.A_TESTER,
    portee=Portee.REP, critere="setup",
    question="Is the start smooth, or is the bar jerked off the floor?",
    etats=(Etat("progressif", "The bar accelerates smoothly out of the floor.", 3),
           Etat("brusque", "The bar is jerked and the lifter is pulled out of position.", 1,
...
L04 = Indicateur(
    ...
    etats=(Etat("oui", "The bar left the floor and was lifted: this is a real repetition."),
           Etat("non", "The bar stayed on the floor, or was already down and the athlete "
                       "simply stood back up: this is NOT a repetition."),
           Etat("inachevee", "The bar left the floor but came back down before lockout: "
    ...
    note_source="LA question que la pose ne peut pas trancher : elle voit le corps, pas la "
                ...
                "'inachevee' est nouveau : une tentative reelle reste analysable, ses "
...
P01 = Indicateur(
    id="P01", nom="derive_poignet", phase=Phase.TIREE, source=Source.POSE,
    portee=Portee.REP, critere="bar_path", vue=Vue.PROFIL,
    question="How far does the hand travel horizontally during the pull?",
    etats=(Etat("proche", "The hands stay over the same point through the pull.", 3),
           Etat("derive_moderee", "The hands drift forward and come back.", 2),
           Etat("derive_forte", "The hands drift well away from the body, lengthening the "
                                "lever on the lower back.", 1, persona="The Pendulum")),
    mesure="derive_ratio", seuils=((0.35, "proche"), (0.70, "derive_moderee"), (INF, "derive_forte")),
...
P02 = Indicateur(
    id="P02", nom="passage_genoux", phase=Phase.TIREE, source=Source.A_TESTER,
    portee=Portee.REP, critere="bar_path",
    question="How does the bar get past the knees?",
    etats=(Etat("direct", "The bar passes the knees close to the legs, in one line.", 3),
           Etat("boucle", "The bar loops forward around the knees before coming back in.", 2),
           Etat("collision", "The bar catches on the knees and the lifter has to work "
                             "around them.", 1)),
...
P03 = Indicateur(
    id="P03", nom="contact_barre_jambes", phase=Phase.TIREE, source=Source.LLM,
    portee=Portee.REP, critere="bar_path",
    question="Does the bar stay in contact with, or very close to, the legs?",
    etats=(Etat("contact", "The bar stays against or within a few centimetres of the legs "
                           "the whole way up.", 3),
           Etat("perte_breve", "Contact is briefly lost, then the bar comes back to the legs.", 3),
           Etat("eloignee", "The bar travels visibly away from the legs.", 1,
...
P04 = Indicateur(
    id="P04", nom="dos_sous_charge", phase=Phase.TIREE, source=Source.LLM,
    ...
    etats=(Etat("inchange", "The back holds the same shape at the floor, at knee height and "
                            "at lockout: no flexion added under load.", 3),
           Etat("arrondi_stable", "There is rounding, but it is set from the start and does "
                                  "not get worse during the pull.", 3),
           Etat("flexion_apparait", "Flexion appears during the pull that was not there at "
                                    "the start.", 2),
           Etat("effondrement", "The lower back rounds hard and keeps rounding as the bar "
...
P05 = Indicateur(
    id="P05", nom="valgus_genoux", phase=Phase.TIREE, source=Source.POSE,
    portee=Portee.REP, critere="leg_drive", vue=Vue.FACE,
    question="Do the knees stay out over the feet, or do they collapse inward?",
    etats=(Etat("suivi", "The knees track outward over the feet throughout.", 3),
           Etat("leger", "The knees waver inward at the hardest point but never collapse.", 3),
           Etat("valgus", "The knees collapse inward off the floor.", 1, persona="The X-Wing")),
    mesure="valgus_ratio", seuils=((0.08, "suivi"), (0.20, "leger"), (INF, "valgus")),
...
P06 = Indicateur(
    id="P06", nom="point_stagnation", phase=Phase.TIREE, source=Source.POSE,
    portee=Portee.REP, critere=None, vue=Vue.PROFIL,
    question="Where does the bar slow down most during the pull?",
    etats=(Etat("aucun", "The pull is continuous, with no marked slowdown."),
           Etat("sol", "The lift is hardest breaking the floor."),
           Etat("genoux", "The lift slows down around knee height."),
           Etat("lockout", "The lift slows down near the top.")),
    mesure="stagnation", seuils=((0.0, "aucun"), (0.34, "sol"), (0.67, "genoux"), (INF, "lockout")),
...
P07 = Indicateur(
    id="P07", nom="duree_tiree", phase=Phase.TIREE, source=Source.POSE,
    portee=Portee.REP, critere=None,
    question="How long does the concentric take?",
    etats=(Etat("rapide", "Under a second: the bar moves easily."),
           Etat("normale", "One to two seconds."),
           Etat("grind", "Over two seconds: a slow grind.")),
    mesure="duree_tiree_s", seuils=((1.0, "rapide"), (2.0, "normale"), (INF, "grind")),
...
P08 = Indicateur(
    ...
    etats=(Etat("non", "The bar rises in one continuous motion.", 3),
           Etat("oui", "The lifter re-flexes the knees and rests the bar on the thighs to "
...
P09 = Indicateur(
    id="P09", nom="asymetrie", phase=Phase.TIREE, source=Source.A_TESTER,
    portee=Portee.REP, critere="lockout", vue=Vue.FACE,
    question="Does one side of the bar rise ahead of the other?",
    etats=(Etat("symetrique", "Both sides rise together.", 3),
           Etat("asymetrique", "One side finishes ahead of the other and the bar rotates.", 2,
...
P10 = Indicateur(
    id="P10", nom="flexion_coude_tiree", phase=Phase.TIREE, source=Source.A_TESTER,
    portee=Portee.REP, critere="setup",
    question="Do the elbows bend during the pull?",
    etats=(Etat("tendus", "The arms stay long throughout the pull.", 3),
           Etat("flechis", "The elbows flex during the pull, pulling with the arms.", 1,
                persona="The T-Rex")),
...
K01 = Indicateur(
    id="K01", nom="extension_hanche", phase=Phase.LOCKOUT, source=Source.POSE,
    ...
    mesure="hanche_lockout_deg", seuils=((160.0, "incomplete"), (INF, "complete")),
)

K02 = Indicateur(
    id="K02", nom="extension_genou", phase=Phase.LOCKOUT, source=Source.POSE,
    portee=Portee.REP, critere="lockout", vue=Vue.PROFIL,
    ...
    mesure="genou_lockout_deg", seuils=((160.0, "incomplete"), (INF, "complete")),
)

K03 = Indicateur(
    id="K03", nom="bascule_arriere", phase=Phase.LOCKOUT, source=Source.POSE,
    portee=Portee.REP, critere="lockout", vue=Vue.PROFIL,
    question="Does the lifter lean back at the top?",
    etats=(Etat("droit", "The lifter finishes upright and neutral.", 3),
           Etat("leger", "A slight lean back at the top.", 3),
    ...
    mesure="bascule_arriere_deg", seuils=((5.0, "droit"), (15.0, "leger"), (INF, "hyperextension")),
...
K04 = Indicateur(
    id="K04", nom="haussement_epaules", phase=Phase.LOCKOUT, source=Source.A_TESTER,
    portee=Portee.REP, critere="lockout",
    question="Does the lifter shrug the shoulders to finish?",
    etats=(Etat("non", "The lift finishes with hip extension alone.", 3),
           Etat("oui", "The lifter shrugs the shoulders at the top: the shrug adds no "
...
K05 = Indicateur(
    id="K05", nom="equilibre_lockout", phase=Phase.LOCKOUT, source=Source.A_TESTER,
    portee=Portee.REP, critere="lockout", vue=Vue.PROFIL,
    question="Where is the lifter's balance at the top?",
    etats=(Etat("stable", "The lifter finishes balanced over the feet.", 3),
           Etat("arriere", "The lifter's weight goes behind the heels and they lean or "
...
K06 = Indicateur(
    id="K06", nom="duree_lockout", phase=Phase.LOCKOUT, source=Source.POSE,
    portee=Portee.REP, critere=None,
    question="How long does the top of the lift take to complete?",
    etats=(Etat("franc", "The lockout is immediate."),
           Etat("lent", "The lockout takes a moment to complete."),
           Etat("bloque", "The lift stalls at the top before finishing.")),
    mesure="duree_lockout_s", seuils=((0.4, "franc"), (1.0, "lent"), (INF, "bloque")),
...
E01 = Indicateur(
    id="E01", nom="initiation_descente", phase=Phase.DESCENTE, source=Source.POSE,
    portee=Portee.REP, critere="descent", vue=Vue.PROFIL,
    question="Which joint bends first on the way down?",
    etats=(Etat("hanches", "The hips travel back first, the knees bending once the bar has "
                           "passed them.", 3),
           Etat("simultane", "Hips and knees bend together.", 3),
           Etat("genoux", "The knees bend before the bar has passed them, pushing the bar "
                          "forward or into the kneecaps.", 2, persona="The Kneecapper")),
    mesure="descente_ordre", seuils=((-5.0, "genoux"), (5.0, "simultane"), (INF, "hanches")),
    note_source="Difference entre la perte d'angle de hanche et celle de genou, 0,35 s "
                "apres le verrouillage. Deja en production.",
)

E02 = Indicateur(
    id="E02", nom="controle_descente", phase=Phase.DESCENTE, source=Source.LLM,
    ...
    etats=(Etat("accompagnee", "The bar is lowered under control, the lifter staying with it.", 3),
           Etat("rapide_accompagnee", "The descent is quick but the hands stay with the bar "
                                      "all the way down.", 3),
           Etat("lachee", "The bar is dropped or crashes to the floor.", 1),
           Etat("hors_champ", "The lowering is cut off by the end of the video.", None)),
    note_source="Il faut voir la barre et le sol : la pose ne voit ni l'un ni l'autre. "
                "L'etat 'hors_champ' est distinct de 'not_visible' : la video s'arrete, "
                "ce n'est pas un probleme de cadrage.",
)

E03 = Indicateur(
    id="E03", nom="enchainement", phase=Phase.DESCENTE, source=Source.LLM,
    portee=Portee.REP, critere=None,
    ...
    etats=(Etat("reset", "The bar comes to a full stop on the floor before the next rep."),
           ...
           Etat("rebond", "The plates bounce off the floor and the bounce is used."),
           Etat("derniere", "This is the last rep of the set.")),
...
CONSEILS = {
    "hauteur_hanches:trop_basses": "Set the hips higher, between the knees and the shoulders, before you pull.",
    "hauteur_hanches:trop_hautes": "Drop the hips a little and bring the shoulders over the bar.",
    "epaules_sur_barre:derriere": "Set the shoulders over or just ahead of the bar before you pull.",
    "epaules_sur_barre:tres_avant": "Bring the hips down slightly so the shoulders sit closer to over the bar.",
    "angle_tibia:inclines": "Sit the hips back until the shins are vertical before you pull.",
    "barre_milieu_pied:trop_avant": "Set the bar over the middle of your foot, close to the shins.",
    "bras_tendus:flechis": "Keep the arms long and let the legs do the work.",
    "flexion_coude_tiree:flechis": "Keep the arms long through the whole pull.",
    "mise_en_tension:partielle": "Pull the slack out until you feel the bar load, then push the floor away.",
    "mise_en_tension:a_coup": "Take the slack out of the bar before you pull instead of yanking it.",
    "a_coup_depart:brusque": "Build tension against the bar, then accelerate: do not snatch it off the floor.",
    "dos_au_setup:lombaire_arrondi": "Set the back flat before the bar moves; drop the load if you cannot hold it.",
    "montee_hanches_epaules:hanches_fusent": "Push the floor away and hold your chest angle through the first third of the pull.",
    "bascule_buste:bascule": "Keep the torso angle as the bar leaves the floor rather than letting the hips win.",
    "derive_poignet:derive_moderee": "Pull the shoulders down and keep the bar tracking over the mid-foot.",
    "derive_poignet:derive_forte": "Keep the bar against your legs: pull the shoulders down and drag it up the shins.",
    "contact_barre_jambes:eloignee": "Keep the bar in contact with the legs the whole way up.",
    "passage_genoux:boucle": "Let the hips come through as the bar reaches the knees so it passes close.",
    "passage_genoux:collision": "Sit the hips back a touch at the knees so the bar has a path.",
    "dos_sous_charge:flexion_apparait": "Brace before you pull, and end the set when the shape starts to change.",
    "dos_sous_charge:effondrement": "Stop the set. Rebuild this at a load where the back holds its shape.",
    "valgus_genoux:valgus": "Push the knees out over your toes as you drive off the floor.",
    "hitch:oui": "Finish with one continuous hip extension instead of ratcheting the bar up the thighs.",
    "asymetrie:asymetrique": "Film a front view and check whether one side is leading before changing anything.",
    "extension_hanche:incomplete": "Finish standing tall, hips and knees locked together.",
    "extension_genou:incomplete": "Lock the knees at the top instead of leaving them soft.",
    "bascule_arriere:hyperextension": "Finish tall by squeezing the glutes, not by leaning back.",
    "haussement_epaules:oui": "Finish with the hips: the shrug adds no height to the bar.",
    "equilibre_lockout:arriere": "Finish balanced over your feet rather than drifting behind your heels.",
    "initiation_descente:genoux": "Send the hips back first and let the knees bend once the bar has passed them.",
    "controle_descente:lachee": "Stay with the bar on the way down instead of dropping it.",
}
